...
import { generate, Output, PalindromeResult } from './palindrome-products.ts'

describe('Palindromes', () => {
  it('smallest palindrome from single digit factors', () => {
    const palindromes: Output = generate({ maxFactor: 9, minFactor: 1 })
    const smallest: PalindromeResult | null = palindromes.smallest
  ...
  it('largest palindrome from single digit factors', () => {
    ...
    const largest: PalindromeResult | null = palindromes.largest
    const expected = {
      value: 9,
      factors: [
        [1, 9],
